...
const cargarDatos = async () => {
  try {
    ...
    console.log("Datos recibidos de Inventario:", inv); // Para depurar en consola
    setProductos(inv.productos || inv || []); // Flexibilidad si el array viene directo
    
    const hist = await getHistorialVentasService();
    setHistorial(hist.datos || []);
  } catch (err) {
    setMensaje({ texto: "Error de conexión con los servicios", tipo: 'error' });
  ...
  const agregarAlCarrito = (prod) => {
    ...
    if (existe) {
      ...
      // Inicializamos el precio con el precio sugerido si existe en el inventario
  ...
  const handleVender = async () => {
    ...
    // Validación básica de stock antes de enviar
        ...
        <table style={tableStyle}>
          <thead>
            <tr style={{ ...headerTable, backgroundColor: '#333', color: 'white' }}>
              ...
              <th style={thStyle}>Productos Vendidos (Resumen)</th>
              <th style={thStyle}>Total</th>
            </tr>
          </thead>
          <tbody>
            {Object.values(historial.reduce((acc, current) => {
              if (!acc[current.id]) {
              ...
              <tr key={v.id} style={trHover}>
                <td style={{ ...tdStyle, fontFamily: 'monospace', fontWeight: 'bold' }}>#{v.id.toString().slice(-6)}</td>
                <td style={tdStyle}>{new Date(v.fecha_hora).toLocaleString()}</td>
                <td style={tdStyle}>
                ...
                <td style={tdStyle}><strong>${Number(v.monto_total).toLocaleString()}</strong></td>
              </tr>
            ))}
            {historial.length === 0 && (
                <tr>
                    <td colSpan="4" style={{ textAlign: 'center', padding: '20px', color: '#999' }}>No hay ventas registradas hoy.</td>
                </tr>
            )}
          </tbody>
        </table>
